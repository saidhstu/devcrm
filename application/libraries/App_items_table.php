...
class App_items_table extends App_items_table_template
{
    ...
    public function items()
    {
        ...
        foreach ($this->items as $item) {
             ...
             * Item custom fields
             */
             unset($customFieldsItems[1]);
             //echo"<pre>";print_r($customFieldsItems);die();
            foreach ($customFieldsItems as $custom_field) {
                $itemHTML .= '<td align="left" width="18%">' . get_custom_field_value($item['id'], $custom_field['id'], 'items') . '</td>';
    ...
    public function html_headings()
    {
        ...
        foreach ($customFieldsItems as $cf) {
            if($cf['slug']!='items_date_of_birth'){
    ...
    //    $tblhtml .= '<th width="' . $descriptionItemWidth . '%" align="left">' . $this->item_heading() . '</th>';
        ...
        foreach ($customFieldsItems as $cf) {
            if($cf['slug']!='items_date_of_birth'){
                $tblhtml .= '<th width="18%" align="left">' . $cf['name'] . '</th>';
            }
        }
