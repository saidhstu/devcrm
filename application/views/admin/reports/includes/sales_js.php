...
<script>
 ...
 var report_airlines = $('#airlines-report');
 ...
 var fnServerParams = {
   ...
   "sale_airlines": '[name="sale_airlines"]',
 }
 $(function() {
   ...
   $('.table-airlines-report').on('draw.dt', function() {
     var airlinesReportsTable = $(this).DataTable();
     var sums = airlinesReportsTable.ajax.json().sums;
     add_common_footer_sums($(this),sums);
     $(this).find('tfoot td.amount_open').html(sums.amount_open);
     $(this).find('tfoot td.applied_credits').html(sums.applied_credits);
     <?php foreach($invoice_taxes as $key => $tax){ ?>
 ...
 function init_report(e, type) {
   ...
   // Clear custom date picker
       ...
       } else if (type == 'airlines-report') {
         report_airlines.removeClass('hide');
   ...
   function airlines_report() {
     if ($.fn.DataTable.isDataTable('.table-airlines-report')) {
       $('.table-airlines-report').DataTable().destroy();
     }
     initDataTable('.table-airlines-report', admin_url + 'reports/airlines_report', false, false, fnServerParams, [
       [2, 'desc'],
       [0, 'desc']
       ]).column(2).visible(false, false).columns.adjust();
   }
 ...
 function items_report(){
   ...
   function gen_reports() {
     ...
     if (!$('.chart-income').hasClass('hide')) {
       ...
       invoices_report();
     } else if (!report_airlines.hasClass('hide')) {
       airlines_report();
