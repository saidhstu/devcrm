...
//// this is new code of vicky ////
    $this->ci->db->where('rel_id',$aRow['id']);
    $this->ci->db->where('rel_type','invoice');
    $iteminfo =$this->ci->db->get('tblitemable');
    $iteminfo = $iteminfo->result_array();
    $ticketns = array();
    foreach ($iteminfo as $key => $result) {
            //print_r($result['rel_id']);die();
        $this->ci->db->where('relid',$result['id']);
        $this->ci->db->where('fieldid','11');
        $this->ci->db->where('fieldto','items');
        $iteminfo2 =$this->ci->db->get('tblcustomfieldsvalues');
        $iteminfo2 = $iteminfo2->result_array();
        foreach ($iteminfo2 as $key => $result2) {
            $ticketns[] = $result2['value'];
        }
    }
    $row[] = implode('<br/>',$ticketns);
    
    $this->ci->db->where('rel_id',$aRow['id']);
    $this->ci->db->where('rel_type','invoice');
    $iteminfo3 =$this->ci->db->get('tblitemable');
    $iteminfo3 = $iteminfo3->result_array();
    $ndate = array();
    foreach ($iteminfo3 as $key => $result3) {
            //print_r($result['rel_id']);die();
        $this->ci->db->where('relid',$result3['id']);
        $this->ci->db->where('fieldto','items');
        $iteminfo4 =$this->ci->db->get('tblcustomfieldsvalues');
        $iteminfo4 = $iteminfo4->result_array();
        $a=0;
        foreach ($iteminfo4 as $key => $result4) {
            
            if($result4['fieldid']=='16' || $result4['fieldid']=='17'){
                $a++;
                
                if($result4['fieldid']=='16'){
                    $ddate = $result4['value'];
                }
                if($result4['fieldid']=='17'){
                    $rdate = $result4['value'];
                }
               // if(strtotime($rdate)!=strtotime($ddate)){
                //print_r($result4);die();
                if($a>0 && $a%2!=1){
                $ndate[] = $ddate.'|'.$rdate;
                }
