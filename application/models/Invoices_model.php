...
class Invoices_model extends App_Model
{
    ...
    public function add($data, $expense = false)
    {
        ...
        $data['insurance_detail'] = trim($data['insurance_detail']);
        $data['insurance_detail'] = nl2br($data['insurance_detail']);
    ...
    {
        $this->db->where('slug', 'invoice_air_lines');
        $this->db->where('fieldto', 'invoice');
        $result = $this->db->get(db_prefix() . 'customfields')->row_array();

        $data['options'] = explode(',', $result['options']);
        $data['invoice_air_lines'] = $result;

        return $data;
    }

    public function invoice_airlines($id)
    {
        $this->db->where('fieldid', $id);
        $this->db->where('fieldto', 'invoice');
        $result = $this->db->get(db_prefix() . 'customfieldsvalues')->result_array();

        foreach($result as $each){
            $item[$each['relid']] = $each['value'];
        }
