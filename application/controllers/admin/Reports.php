...
class Reports extends AdminController
{
    ...
    public function sales()
    {
        ...
        $data['airlines'] = $this->invoices_model->get_airlines();
    ...
    public function test_report()
    {
        ...
        $this->load->view('admin/reports/test_report', $data);
    }
    
    public function segments_report()
    {
        $data['title']  = "segment report test";
        $data['segments'] = $this->reports_model->get_segments();
        
        $this->load->view('admin/reports/segments_report', $data);
    }

    public function airlines_report()
    {
        if ($this->input->is_ajax_request()) {
            $invoice_taxes     = $this->distinct_taxes('invoice');
            $totalTaxesColumns = count($invoice_taxes);
            ...
            $this->load->model('invoices_model');
            $output['airlines'] = $this->invoices_model->get_airlines();
            $invoice_airlines = $output['invoice_airlines'] = $this->invoices_model->invoice_airlines($output['airlines']['invoice_air_lines']['id']);

            $sale_airlines = [];
            if ($this->input->post('sale_airlines')) {
                $sale_airlines  = $this->input->post('sale_airlines');
               
            }

            foreach ($rResult as $aRow) {

                $row_airline = @$invoice_airlines[$aRow['id']];

                if(!empty($sale_airlines) && !in_array($row_airline, $sale_airlines))continue;
